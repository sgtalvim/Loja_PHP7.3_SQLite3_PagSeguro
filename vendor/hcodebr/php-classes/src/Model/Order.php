...
class Order extends Model {
	...
	*/
	...
	// serve para criar uma nova ordem de compra ou atualizar uma existente
	public function save()
	{
		...
		if(!isset($idorder)){
			...
			$results = $sql->query("INSERT INTO tb_orders (idcart, iduser, idstatus, idaddress, vltotal, dtregister) VALUES (:idcart, :iduser, :idstatus, :idaddress, :vltotal, :dtregister)", [
				':idcart'=>$this->getidcart(),
				':iduser'=>$this->getiduser(),
				':idstatus'=>$this->getidstatus(),
				':idaddress'=>$this->getidaddress(),
			...
			$idorder = $sql->select("SELECT idorder FROM tb_orders WHERE idcart = :idcart AND dtregister = :dtregister", array(
				":idcart"=>$this->getidcart(),
				":dtregister"=>$this->getdtregister()
			));

			$this->setidorder($idorder[0]["idorder"]);

		} else {

			$sql->query("UPDATE tb_orders SET idcart = :idcart, iduser = :iduser, idstatus = :idstatus, idaddress = :idaddress, vltotal = :vltotal WHERE idorder = :idorder", array(
				':idcart'=>$this->getidcart(),
				':iduser'=>$this->getiduser(),
				':idstatus'=>$this->getidstatus(),
				':idaddress'=>$this->getidaddress(),
	...
	*/
	...
	// método para o SQLite, sem procedure
	public function setPagSeguroTransactionRespose(
		string $descode, 
		float $vlgrossamount,
		float $vldisccountamount,
	...
	)
	{
		...
		if(isset($idorder)){
			$sql->query("DELETE FROM tb_orderspagseguro WHERE idorder = :idorder", [
				':idorder'=>$this->getidorder()
			]);

			$sql->query("INSERT INTO tb_orderspagseguro (idorder, descode, vlgrossamount, vldiscountamount, vlfeeamount, vlnetamount, vlextraamount, despaymentlink) VALUES (:idorder, :descode, :vlgrossamount, :vldisccountamount, :vlfeeamont, :vlnetamount, :extraamount, :despaymentlink)", [
				':idorder'=>$this->getidorder(),
				':descode'=>$descode,
				':vlgrossamount'=>$vlgrossamount,
				':vldisccountamount'=>$vldisccountamount,
